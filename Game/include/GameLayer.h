...
public: 
	...
	void OnUpdate(float dt) override
	{
		...
		if (Thrive::Input::IsKeyDown(Thrive::Key::A))
			m_Camera.Move(-speed * dt, 0.f);

		if (Thrive::Input::IsKeyDown(Thrive::Key::D))
			m_Camera.Move(speed * dt, 0.f);
	...
	void OnEvent(Thrive::Event& e) override
	{
		...
		dispatcher.Dispatch<Thrive::KeyPressed>([](Thrive::KeyPressed&)
			{
				// only use for single actions like UI toggles
				return false;
			});
	}
